...
const sourceDir = join(root, "packages/chat-app");
const templateDir = join(root, "packages/droplet/templates/chat");
...
console.log(`Promoted chat template to ${relative(root, templateDir)}`);
...
function makeTemplateTsconfig(): Record<string, unknown> {
  return {
    ...
    include: ["src", "tests", "scripts", "alchemy.run.ts", "vitest.config.ts"],
  };
}
...
async function rewriteReadme(): Promise<void> {
  ...
  const rewritten = readme
    .replaceAll("bun run --cwd packages/chat-app deploy", "bun run deploy")
    .replaceAll("bun run --cwd packages/chat-app check", "bun run check")
    .replaceAll("bun run --cwd packages/chat-app test", "bun run test")
...
async function validateTemplate(): Promise<void> {
  ...
  if (failures.length > 0) {
    throw new Error(`Generated chat template contains forbidden monorepo references:\n${failures.join("\n")}`);
  }
}
