...
const sourceDir = join(root, "packages/tasks-app");
const templateDir = join(root, "packages/droplet/templates/tasks");
...
console.log(`Promoted tasks template to ${relative(root, templateDir)}`);
...
function makeTemplateTsconfig(): Record<string, unknown> {
  return {
    ...
    include: ["src", "tests", "alchemy.run.ts", "vitest.config.ts"],
  };
}
...
async function rewriteReadme(): Promise<void> {
  ...
  const rewritten = readme
    .replaceAll("bun run --cwd packages/tasks-app deploy", "bun run deploy")
    .replaceAll("bun run --cwd packages/tasks-app check", "bun run check")
    .replaceAll("bun run --cwd packages/tasks-app test", "bun run test")
...
async function validateTemplate(): Promise<void> {
  ...
  if (failures.length > 0) {
    throw new Error(`Generated tasks template contains forbidden monorepo references:\n${failures.join("\n")}`);
  }
}
