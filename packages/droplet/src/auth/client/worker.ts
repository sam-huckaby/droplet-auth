...
export function createLogoutResponse(options: { cookieName?: string; redirectTo?: string } = {}): Response {
  return new Response(null, {
    status: 303,
    headers: {
      location: options.redirectTo ?? "/",
      "set-cookie": `${options.cookieName ?? "da_session"}=; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Max-Age=0; Path=/; HttpOnly; Secure; SameSite=Lax`,
    },
  });
}
