...
Generate it from this monorepo with:

```sh
bun run --cwd packages/auth-app generate:key
```

In a generated standalone auth app, run:
...
bun run --cwd packages/auth-app/examples/protected-worker alchemy deploy ./alchemy.run.ts
```
...
`packages/droplet/templates/auth/README.md` is generated from this README by the auth template promotion script. Update this file first, then promote the template:
